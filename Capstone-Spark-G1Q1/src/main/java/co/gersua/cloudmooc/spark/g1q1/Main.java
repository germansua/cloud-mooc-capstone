...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Expected a input folder
        JavaRDD<String> lines = sc.textFile(args[0]);
        JavaRDD<String> airports = lines.flatMap(line -> Arrays.asList(line.split(SPLIT_PATTERN)));
        JavaPairRDD<String, Long> airportsMap = airports.mapToPair(airport -> new Tuple2<>(airport.replaceAll("\"", ""), 1L));
        JavaPairRDD<String, Long> airportsReduce = airportsMap.reduceByKey((accumulatedVal, newVal) -> accumulatedVal + newVal);
