...
public class BestFlightReducer extends Reducer<Text, Text, Text, Text> {
    ...
    public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        ...
        List<FlightInfo> bestOrgPerformanceList = new ArrayList<FlightInfo>();
        List<FlightInfo> bestDstPerformanceList = new ArrayList<FlightInfo>();
        ...
        for (Text value : values) {
            ...
            if (flightInfo.getType().equals("ORG")) {
                bestOrgPerformanceList.add(flightInfo);
            } else {
                bestDstPerformanceList.add(flightInfo);
            }
        }

        if (bestOrgPerformanceList.isEmpty() || bestDstPerformanceList.isEmpty()) {
            return;
        }

        for (FlightInfo bestOrgPerformance : bestOrgPerformanceList) {
            for (FlightInfo bestDstPerformance : bestDstPerformanceList) {
                ...
                String outputKey = String
                        .format("%s->%s->%s", bestDstPerformance.getValueAirport(), bestDstPerformance.getKeyAirport(),
                                bestOrgPerformance.getValueAirport());

                String outputValue = String
                        .format("%s\t%d\t%d\t%s\t%d\t%d", DATE_FORMAT.format(bestOrgPerformance.getFlightDate()),
                                bestOrgPerformance.getDepartureTime(), bestOrgPerformance.getArrivalDelay(),
                                DATE_FORMAT.format(bestDstPerformance.getFlightDate()),
                                bestDstPerformance.getDepartureTime(), bestDstPerformance.getArrivalDelay());
